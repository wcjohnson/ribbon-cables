...
---@class ribbon_cables.PinConnection: Core.WorldState
---@field public wire_connector_id defines.wire_connector_id

---@param stored_connections ribbon_cables.PinConnection[]
---@param connections WireConnection[]
local function save_connections(stored_connections, connections)
	for _, connection in pairs(connections) do
		...
		then
			local ws = get_world_state(connection.target.owner) --[[@as ribbon_cables.PinConnection]]
			ws.wire_connector_id = connection.target.wire_connector_id
			stored_connections[#stored_connections + 1] = ws
		end
	end
end
...
---@param stored_connections ribbon_cables.PinConnection[]
local function restore_connections(connector, stored_connections)
	for _, ws in pairs(stored_connections) do
		local target_entity = find_matching(ws)[1]
		if target_entity then
			local target_connector =
				target_entity.get_wire_connector(ws.wire_connector_id, true)
			if target_connector then connector.connect_to(target_connector, true) end
		end
	end
end
...
event.bind(
	...
	function(ev)
		...
		local reds = pin_entity.get_wire_connector(
			defines.wire_connector_id.circuit_red,
			false
		)
		local greens = pin_entity.get_wire_connector(
			defines.wire_connector_id.circuit_green,
			false
		)
		local red_connections = {}
		local green_connections = {}
		if reds then save_connections(red_connections, reds.connections) end
		if greens then save_connections(green_connections, greens.connections) end
		remote.call(
			"things",
			"set_transient_data",
			ev.id,
			"connections",
			{ red = red_connections, green = green_connections }
		)
	end
)
...
event.bind(
	...
	function(ev)
		...
		local red_connections = transient_data.connections.red
		if next(red_connections) then
			local reds = pin_entity.get_wire_connector(
				defines.wire_connector_id.circuit_red,
				true
			)
			restore_connections(reds, red_connections)
		end
		local green_connections = transient_data.connections.green
		if next(green_connections) then
			local greens = pin_entity.get_wire_connector(
				defines.wire_connector_id.circuit_green,
				true
			)
			restore_connections(greens, green_connections)
		end
	end
)
